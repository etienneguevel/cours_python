...
class Point:
    ...
    def set_x(self, value):
        if type(value) not in [int, float]:
            raise TypeError(f"{type(value)} is a wrong type for x argument.")
        self.x = value
\end{lstlisting}
...
>>> point = Point(1, 2)
>>> point.set_x("azerty")
TypeError: str is a wrong type for x argument. 
\end{lstlisting}
...
\begin{frame}[fragile]{\texttt{@attr.setter}}
\begin{lstlisting}[language=Python, numbers=none]
class Point:
  def __init__(self, x, y):
      self.__x = x
...
L'attribut \texttt{x} peut désormais être modifié, et son type est vérifié avant. 
